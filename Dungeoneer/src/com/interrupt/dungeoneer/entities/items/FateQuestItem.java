...
public class FateQuestItem extends Item {

    public FateQuestItem() {
        super(0, 0, 59, ItemType.quest, StringManager.get("items.FateQuestItem.defaultNameText"));
    }

    public FateQuestItem(float x, float y) {
        super(x, y, 59, ItemType.quest, StringManager.get("items.FateQuestItem.defaultNameText"));
    }
    ...
    protected void pickup(Player player)
    {
        ...
        if(Game.instance.player.addToInventory(this))
        {
            ...
            Game.ShowMessage(StringManager.get("items.FateQuestItem.noRoomText"), 1);
        }
    }

    public void doQuestThing() {

        if(Game.instance.player.isHoldingFateQuestItem) return;

        Game.ShowMessage(MessageFormat.format(StringManager.get("items.FateQuestItem.gotItemText"), GetName()), 8, 1f);

        Game.message.clear();

        if(showPickupMessage) {
            String escapeMessage = StringManager.get("items.FateQuestItem.escapeMessageText");
        ...
        // make life difficult
        Game.instance.player.isHoldingFateQuestItem = true;
